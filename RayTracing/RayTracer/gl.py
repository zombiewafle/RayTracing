...
def reflectVector(normal, dirVector):
    # R = 2 * ( N . L) * N - L
    reflect = 2 * ml.dotProduct(normal, dirVector)
    reflect = ml.multyMatrix(reflect, normal)
    reflect = ml.subVectors(reflect, dirVector)
...
class Raytracer(object):
    ...
    def cast_ray(self, orig, dir, origObj = None, recursion = 0):
        ...
        view_dir = view_dir / ml.norm(view_dir)
        ...
        if self.dirLight:
            ...
            finalSpecColor = ml.sumVectors(finalSpecColor, (1 - shadow_intensity) * specColor)
        ...
        for pointLight in self.pointLights:
            ...
            light_dir = light_dir / ml.norm(light_dir)
            ...
            # Shadows
            shadInter = self.scene_intersect(intersect.point, light_dir, intersect.sceneObject)
            lightDistance = ml.norm( ml.subVectors(pointLight.position, intersect.point) )
            if shadInter and shadInter.distance < lightDistance:
                shadow_intensity = 1

            pLightColor = ml.sumVectors(pLightColor, (1 - shadow_intensity) * diffuseColor)
            finalSpecColor = ml.sumVectors(finalSpecColor, (1 - shadow_intensity) * specColor)
